...
const memoryDir = path.join(dataDir, 'memory'); // Dream memory consolidation directory
const workspacesBaseDir = path.join(app.getPath('documents'), 'Could-I-Be-Your-Partner');

[dataDir, imagesDir, skillsDir, historyDir, babeHistoryDir, memoryDir, workspacesBaseDir].forEach(d => { if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); });

// Initialize memory directory with topics.md if absent
const memoryIndexPath = path.join(memoryDir, 'topics.md');
if (!fs.existsSync(memoryIndexPath)) {
  fs.writeFileSync(memoryIndexPath, `# Memory Index\n\n> Auto-generated by Could I Be Your Partner.\n> This file is maintained by the Dream skill.\n\n`, 'utf-8');
}
const dreamStatePath = path.join(memoryDir, 'dream-state.json');

const settingsPath = path.join(dataDir, 'settings.json');
const memoryPath = path.join(dataDir, 'memory.json');
const knowledgePath = path.join(dataDir, 'knowledge.json');

// Dream state: tracks last consolidation time, session count, and PID lock.
function loadDreamState() {
  try { return JSON.parse(fs.readFileSync(dreamStatePath, 'utf-8')); }
  catch { return { lastConsolidatedAt: 0, sessionsSinceConsolidation: 0, lockPid: null, lockAt: 0 }; }
}
function saveDreamState(state) {
  fs.writeFileSync(dreamStatePath, JSON.stringify(state, null, 2), 'utf-8');
}
function tryAcquireDreamLock() {
  const state = loadDreamState();
  const now = Date.now();
  // Lock expires after 1 hour (prevents stale PID reuse)
  if (state.lockPid && (now - state.lockAt) < 3600000) {
    try { process.kill(state.lockPid, 0); /* still alive */ return false; }
    catch { /* process dead — steal lock */ }
  }
...
function releaseDreamLock() {
  const state = loadDreamState();
  if (state.lockPid === process.pid) {
    state.lockPid = null;
    state.lockAt = 0;
    saveDreamState(state);
  }
}
function recordDreamConsolidation() {
  const state = loadDreamState();
...
// ---- IPC: Dream (memory consolidation) ----
ipcMain.handle('dream:getState', () => loadDreamState());
ipcMain.handle('dream:acquireLock', () => tryAcquireDreamLock());
ipcMain.handle('dream:releaseLock', () => { releaseDreamLock(); return true; });
ipcMain.handle('dream:recordConsolidation', () => { recordDreamConsolidation(); return true; });
ipcMain.handle('dream:incrementSession', () => {
  const state = loadDreamState();
  state.sessionsSinceConsolidation = (state.sessionsSinceConsolidation || 0) + 1;
  saveDreamState(state);
  return state;
});
ipcMain.handle('dream:checkGate', () => {
  // Triple gating: time + sessions + lock
  const agent = settings.agent || {};
  const enabled = agent.autoDreamEnabled !== false;
  if (!enabled) return { passed: false, reason: 'disabled' };
  const state = loadDreamState();
  const minHours = agent.autoDreamMinHours ?? 24;
  const minSessions = agent.autoDreamMinSessions ?? 5;
  const hoursSince = state.lastConsolidatedAt
    ? (Date.now() - state.lastConsolidatedAt) / 3600000
    : Infinity;
  if (hoursSince < minHours) return { passed: false, reason: 'time', hoursSince: Math.round(hoursSince * 10) / 10, minHours };
  if ((state.sessionsSinceConsolidation || 0) < minSessions) {
    return { passed: false, reason: 'sessions', sessions: state.sessionsSinceConsolidation || 0, minSessions };
  }
  if (!tryAcquireDreamLock()) return { passed: false, reason: 'locked' };
  return { passed: true, state };
});
ipcMain.handle('dream:getMemoryDir', () => memoryDir);

// Append a JSONL record of a conversation turn to today's session file.
// Dream reads these session_*.jsonl files to consolidate persistent memory.
ipcMain.handle('dream:appendSession', (_, record) => {
  try {
    if (!record || typeof record !== 'object') return { ok: false, error: 'invalid record' };
    const now = new Date();
    const dateStr = now.toISOString().slice(0, 10); // YYYY-MM-DD
    const sessionFile = path.join(memoryDir, `session_${dateStr}.jsonl`);
    const line = JSON.stringify({
      ts: record.ts || now.getTime(),
      iso: record.iso || now.toISOString(),
      conversationId: record.conversationId || null,
      title: record.title || '',
      user: record.user || '',
      assistantTurns: Array.isArray(record.assistantTurns) ? record.assistantTurns : []
    }) + '\n';
    fs.appendFileSync(sessionFile, line, 'utf-8');
    return { ok: true, file: sessionFile };
  } catch (e) { return { ok: false, error: e.message }; }
});
