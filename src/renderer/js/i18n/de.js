...
_textMap: {
  ...
  // ── Einstellungsfelder: Budget ──
  '预算周期设置': 'Budgetzeitraum-Einstellungen',
  '配置日/周/月预算的统计周期边界与时区。': 'Konfiguriert die Zyklusgrenzen und Zeitzone für Tages-/Wochen-/Monatsbudgets.',
  '时区': 'Zeitzone',
  '影响日/周/月起止时间': 'Beeinflusst die Tages-/Wochen-/Monatsgrenzen',
  '周周期': 'Wochenzyklus',
  '自然周（周一起）': 'Natürliche Woche (ab Montag)',
  '滚动 7 天': 'Rollen 7 Tage',
  '自然周从周一 00:00 开始': 'Natürliche Woche startet Montag 00:00',
  '月周期': 'Monatszyklus',
  '自然月（1 日起）': 'Natürlicher Monat (ab dem 1.)',
  '滚动 30 天': 'Rollen 30 Tage',
  '自然月从每月 1 日 00:00 开始': 'Natürlicher Monat startet am 1. um 00:00',
  '预算上限': 'Budgetobergrenzen',
  '设定每日/每周/每月消费上限（USD）。超过时按下方策略处理。': 'Legen Sie Tages-/Wochen-/Monatslimits (USD) fest. Bei Überschreitung greift die untenstehende Richtlinie.',
  '每日上限 (USD)': 'Tageslimit (USD)',
  '每周上限 (USD)': 'Wochenlimit (USD)',
  '月度上限 (USD)': 'Monatslimit (USD)',
  '0 = 不限制': '0 = unbegrenzt',
  '超额策略': 'Über-Budget-Richtlinie',
  '仅警告（继续使用主模型）': 'Nur warnen (Hauptmodell weiter verwenden)',
  '切换到 fallback 模型': 'Zu Fallback-Modell wechseln',
  '停止接受新请求': 'Neue Anfragen ablehnen',
  'Fallback 模型': 'Fallback-Modell',
  '从可用模型列表选择': 'Aus der verfügbaren Modellliste wählen',
  ...
  '消费趋势': 'Ausgabentrend',
  '预算占比': 'Budgetanteil',
  '模型价格表': 'Modellpreistabelle',
  '为每个模型填写每 1M token 的单价（USD）。': 'Geben Sie den Preis pro 1M Token (USD) für jedes Modell ein.',
  '分别支持输入、缓存读取（cache read）、输出、缓存写入（cache write）。': 'Unterstützt separat Eingabe, Cache-Read, Ausgabe und Cache-Write.',
  '缓存写入仅勾选 "支持" 的模型才会计费（如 Claude 系列）；其他模型即使有 cacheCreation token 也不计费。': 'Cache-Write wird nur bei Modellen mit "unterstützt" berechnet (z. B. Claude); andere Modelle werden auch mit cacheCreation-Tokens nicht berechnet.',
  '上下文模态框的费用显示依赖此价格表，未填写的模型不显示费用。': 'Die Kostenanzeige im Kontextmodus hängt von dieser Tabelle ab; Modelle ohne Preis zeigen keine Kosten.',
  '模型 ID': 'Modell-ID',
  '输入 $/1M': 'Eingabe $/1M',
  '缓存读 $/1M': 'Cache-Read $/1M',
  '输出 $/1M': 'Ausgabe $/1M',
  '缓存写 $/1M': 'Cache-Write $/1M',
  '支持 CW': 'CW unterstützt',
  '添加模型价格': 'Modellpreis hinzufügen',
  '从当前 LLM 配置导入': 'Aus aktueller LLM-Konfiguration importieren',
  '从用量记录导入已用模型': 'Verwendete Modelle aus Verbrauchsaufzeichnungen importieren',
  '峰谷时段定价': 'Haupt-/Nebenzeiten-Preise',
  '启用后在指定时段内对各类价格应用倍率。例如某些 API 在高峰时段按 1.5x 计费。': 'Wenn aktiviert, werden in den angegebenen Zeiten Multiplikatoren angewendet. Manche APIs berechnen z. B. 1,5x in Spitzenzeiten.',
  ...
  '输入倍率': 'Eingabe-Multiplikator',
  '缓存读倍率': 'Cache-Read-Multiplikator',
  '输出倍率': 'Ausgabe-Multiplikator',
  '缓存写倍率': 'Cache-Write-Multiplikator',
  '预算状态': 'Budgetstatus',

  // ── Einstellungsfelder: Benachrichtigungen ──
  '系统通知': 'Systembenachrichtigungen',
  '在需要您干预时发送系统桌面通知（敏感操作审批、会话完成、问卷、文件呈递等）。': 'Sendet Desktop-Benachrichtigungen, wenn Ihr Eingreifen nötig ist (Genehmigung sensibler Aktionen, Abschluss der Sitzung, Fragebögen, Dateiüberreichung usw.).',
  '关闭后所有通知都不再弹窗（但 App 内的消息提示仍会显示）。': 'Wenn deaktiviert, erscheinen keine Popups mehr (In-App-Hinweise werden weiterhin angezeigt).',
  '启用系统通知': 'Systembenachrichtigungen aktivieren',
  '通知类型': 'Benachrichtigungstypen',
  '敏感操作审批请求': 'Genehmigungsanfragen für sensible Aktionen',
  '如删除文件、执行危险命令等需要您批准时': 'Wenn eine Genehmigung nötig ist, z. B. beim Löschen von Dateien oder Ausführen gefährlicher Befehle',
  '当前会话已完成': 'Aktuelle Sitzung abgeschlossen',
  'Agent 结束一轮工作并等待下一条指令时': 'Wenn der Agent eine Arbeitsrunde beendet und auf die nächste Anweisung wartet',
  '问卷（askQuestions）': 'Fragebögen (askQuestions)',
  'Agent 向您询问选择题/填空题时': 'Wenn der Agent Multiple-Choice-/Ausfüllfragen stellt',
  '文件呈递（presentFile）': 'Dateiüberreichung (presentFile)',
  'Agent 想给您看一个文件内容时': 'Wenn der Agent Ihnen den Inhalt einer Datei zeigen möchte',
  'Babe 主动消息': 'Babe-Nachrichten',
  'Babe 主动发来消息时通知您（即便您正在其他模式）': 'Benachrichtigt Sie, wenn Babe von sich aus schreibt (auch in einem anderen Modus)',
  '发送测试通知': 'Testbenachrichtigung senden',

  // ── Einstellungsfelder: Terminal ──
  '终端策略': 'Terminal-Richtlinie',
  '配置 Abort（停止）聊天时，对正在运行的终端如何处理。': 'Legt fest, was mit laufenden Terminals passiert, wenn ein Chat abgebrochen (Abort) wird.',
  'Abort 聊天时的终端策略': 'Terminal-Richtlinie bei Chat-Abbruch',
  '直接掐断运行中的终端（默认）': 'Laufendes Terminal sofort beenden (Standard)',
  '传入 Ctrl+C（保留终端会话）': 'Ctrl+C senden (Terminalsitzung behalten)',
  '不管，让终端继续运行': 'Nichts tun, Terminal weiterlaufen lassen',
  '「终结断」会同时结束终端内的进程；「Ctrl+C」相当于手动按键中止当前命令；「不管」则完全不动终端。': '"Beenden" beendet auch Prozesse im Terminal; "Ctrl+C" stoppt den aktuellen Befehl wie ein manueller Tastendruck; "Nichts tun" lässt das Terminal unberührt.',
  'Shell 选择': 'Shell-Auswahl',
  '手动指定终端使用的 Shell。不熟悉时建议保持「自动检测」。': 'Shell für das Terminal manuell festlegen. Bei Unsicherheit "Automatische Erkennung" lassen.',
  '自动检测（推荐）': 'Automatische Erkennung (empfohlen)',
  '自定义 Shell…': 'Benutzerdefinierte Shell…',
  '自定义 Shell 路径': 'Pfad zur benutzerdefinierten Shell',
  '指定 Shell 不存在 or 无效时自动回退到自动检测。': 'Fällt automatisch auf die automatische Erkennung zurück, wenn die angegebene Shell nicht existiert oder ungültig ist.',
  ...
  '已移除自动降级（会话锁定模型后不再自动切换，保护提示词缓存）': 'Automatisches Downgrade entfernt (das sitzungsgebundene Modell wird nie automatisch gewechselt, um den Prompt-Cache zu schützen)',
  ...
  '工具自动选择': 'Automatische Werkzeugauswahl',
  '危险命令护栏': 'Gefahren-Guard für Befehle',
  '游戏决策（猜人物/卧底/三国杀/成语/飞花令）': 'Spielentscheidungen (Personenraten / Undercover / Sanguosha / Idiom / Fliegende Blume)',
  '邮件/热消息意图分类': 'E-Mail-/Hot-Message-Intent-Klassifikation',
  '上下文保留打分': 'Kontext-Beibehaltung-Bewertung',
  'LLM 主动调用（decisionModel 工具）': 'LLM-initiierte Aufrufe (decisionModel-Tool)',
  '允许模型在对话中自行调用决策模型；关闭后决策工具对 LLM 隐藏': 'Erlaubt dem Modell, das Entscheidungsmodell selbst aufzurufen; aus = Tool wird dem LLM verborgen',
  '配置后自动启用': 'Nach Konfiguration automatisch aktiv',
  '决策': 'Entscheidung',
  '启用决策模型': 'Entscheidungsmodell aktivieren',
  '服务源': 'Dienstquelle',
  '置信阈值': 'Konfidenzschwelle',
  '护栏阈值（noul）': 'Guard-Schwelle (noul)',
  '连通性测试': 'Verbindungstest',
