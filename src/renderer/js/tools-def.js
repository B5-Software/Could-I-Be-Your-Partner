...
function getToolSchemas(enabledTools, mode, imOwner) {
  const schemas = {
    ...
    readFile: { type: 'function', function: { name: 'readFile', description: '读取文件内容（返回带行号的内容，便于后续编辑定位）。仅适用于文本文件；严禁读取二进制文件（png/jpg/zip/exe/mp4等，会乱码污染上下文），图片内容用 OCR 工具。自动识别文件编码（chardet）与换行模式（CRLF/CR/LF），返回结果附带 encoding（编码）与 eol（换行模式）；也可手动指定编码（如utf-8、gbk、big5、shift_jis等）。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径（工作目录相对路径或绝对路径）' }, encoding: { type: 'string', description: '手动指定文件编码（如utf-8、gbk、big5、shift_jis等）。不指定则自动检测。' } }, required: ['path'] } } },
    ...
    getFileEncodingInfo: { type: 'function', function: { name: 'getFileEncodingInfo', description: '获取文件的编码与换行模式信息（encoding、eol、size），用于判断文件是否为 GBK/UTF-8/UTF-16 等编码，以及 CRLF/LF 换行。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径（工作目录相对路径或绝对路径）' } }, required: ['path'] } } },
    ...
    listDirectory: { type: 'function', function: { name: 'listDirectory', description: '列出目录内容。渐进式探索：先列当前工作目录或已知子目录，再进入具体子目录；禁止直接列磁盘根目录/用户主目录/系统目录（会阻塞并可能崩溃）。', parameters: { type: 'object', properties: { path: { type: 'string', description: '目录路径（具体子目录）' } }, required: ['path'] } } },
