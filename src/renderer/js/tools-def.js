...
function getToolSchemas(enabledTools, mode) {
  const schemas = {
    ...
    readFile: { type: 'function', function: { name: 'readFile', description: '读取文件内容（返回带行号的内容，便于后续编辑定位）。自动识别文件编码（chardet）与换行模式（CRLF/CR/LF），返回结果附带 encoding（编码）与 eol（换行模式）；也可手动指定编码（如utf-8、gbk、big5、shift_jis等）。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径（工作目录相对路径或绝对路径）' }, encoding: { type: 'string', description: '手动指定文件编码（如utf-8、gbk、big5、shift_jis等）。不指定则自动检测。' } }, required: ['path'] } } },
