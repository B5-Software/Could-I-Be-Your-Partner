...
function getToolSchemas(enabledTools, mode) {
  const schemas = {
    ...
    readFile: { type: 'function', function: { name: 'readFile', description: '读取文件内容（返回带行号的内容，便于后续编辑定位）。自动识别文件编码（chardet），也可手动指定编码（如utf-8、gbk、big5、shift_jis等）。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径（工作目录相对路径或绝对路径）' }, encoding: { type: 'string', description: '手动指定文件编码（如utf-8、gbk、big5、shift_jis等）。不指定则使用chardet自动检测。' } }, required: ['path'] } } },
    editFile: { type: 'function', function: { name: 'editFile', description: '编辑文件（支持字符串替换和全量覆写两种模式）。字符串替换模式：指定 old_string（要查找的原文）和 new_string（替换文本），支持 replace_all 全局替换。全量覆写模式：仅指定 content 参数。读取文件时自动识别编码，也可通过 encoding 参数手动指定。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径（工作目录相对路径或绝对路径）' }, old_string: { type: 'string', description: '要查找并替换的原文（必须与文件内容精确匹配，包括缩进和换行）' }, new_string: { type: 'string', description: '替换后的文本' }, replace_all: { type: 'boolean', description: '是否替换所有匹配项（默认false，仅替换第一个）。当old_string出现多次且未设此选项时将报错。' }, content: { type: 'string', description: '全量覆写模式：直接写入完整新内容（与old_string/new_string互斥）' }, encoding: { type: 'string', description: '手动指定文件编码（如utf-8、gbk、big5、shift_jis等）。不指定则使用chardet自动检测。' } }, required: ['path'] } } },
    multiEditFile: { type: 'function', function: { name: 'multiEditFile', description: '批量编辑文件：对同一文件执行多处字符串替换。按顺序依次应用每个编辑。读取文件时自动识别编码，也可通过 encoding 参数手动指定。', parameters: { type: 'object', properties: { path: { type: 'string', description: '文件路径' }, edits: { type: 'array', items: { type: 'object', properties: { old_string: { type: 'string', description: '要查找的原文' }, new_string: { type: 'string', description: '替换后的文本' }, replace_all: { type: 'boolean', description: '是否替换所有匹配项' } }, required: ['old_string', 'new_string'] }, description: '编辑列表（按顺序依次应用）' }, encoding: { type: 'string', description: '手动指定文件编码（如utf-8、gbk、big5、shift_jis等）。不指定则使用chardet自动检测。' } }, required: ['path', 'edits'] } } },
